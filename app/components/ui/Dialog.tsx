  ...
  return (
    <div>
      <Button
        type="button"
        onClick={openDialog}
        className="rounded-md bg-pink-500 px-4 py-2 text-white hover:bg-pink-600 focus:ring-2 focus:ring-pink-400"
      >
        {triggerButtonText}
      </Button>
      <dialog
        ref={dialogRef}
        className="items-top w-full max-w-4xl space-x-2 rounded-md bg-gray-50  p-6  shadow-lg "
      >
        <div className="mt-4 flex justify-between gap-2">
          <h2 className="mb-4 text-lg font-medium text-pink-700">{dialogTitle}</h2>
          <Button
            type="button"
            onClick={closeDialog}
            className="rounded-md border border-pink-300 px-4 py-2 text-pink-700 hover:bg-pink-100"
          >
            <LuX />
          </Button>
        </div>
